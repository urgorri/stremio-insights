...
describe("Exporters", () => {
  ...
  it("should sanitize fields starting with formula characters to prevent CSV injection", () => {
    ...
    expect(lines[1]).toContain("'-10 @SUM(1,2)");
    expect(lines[1]).toContain("'\tTabbed");
    expect(lines[1]).toContain("'\rCarriage");
  });
});
