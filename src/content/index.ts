import { processSyncPipeline } from "./sync";
...
export async function runSyncPipeline() {
  ...
  try {
    ...
    const cacheHitRate = itemsToFetchCount > 0 ? (cacheHitsCount / itemsToFetchCount) * 100 : 100;

    // Output complete, real-world METADATA COVERAGE and PERFORMANCE VALIDATION reports!
    console.log("=========================================");
    console.log("METADATA COVERAGE REPORT");
    console.log(`- Total library items: ${datastoreMap.size}`);
    ...
    console.log(`- Concurrency level: ${itemsToFetchCount} requests`);
